...
size_t
jdb_set_default_cache_size(size)
     size_t              size;
{
 ...
 **************************************************************************** */
#define DIMDB   256
static JDB_T        rwdb[DIMDB];

static void        *
jdb_periodic_tasks(arg)
     void               *arg;
{
  DB_ENV             *dbenv;
  int                 nerra = 0, nerrb = 0;
...
JDB_ENV_T          *
jdb_env_open(home, rdonly, dt_chkpoint)
     char               *home;
     bool                rdonly;
     int                 dt_chkpoint;
{
  ...
  if (dt_chkpoint > 0 || TRUE)
  {
    ...
    if ((res =
         pthread_create(&ptid, NULL, jdb_periodic_tasks, (void *) dbenv)) != 0)
    {
      errno = res;
      LOG_SYS_ERROR("Failed spawning jdb_periodic_tasks thread: %s\n",
                    strerror(errno));
      exit(1);
    }
  }
...
bool                jdb_check_last_mtime(JDB_T *);
...
bool
jdb_open(h, dbenv, database, mode, rdonly, dbtype, dbcache)
     JDB_T              *h;
     JDB_ENV_T          *dbenv;
     char               *database;
     int                 mode;
 ...
 **************************************************************************** */
bool
jdb_cursor_get_first(h, k, szk, d, szd)
     JDB_T              *h;
     char               *k;
     size_t              szk;
     void               *d;
 ...
 **************************************************************************** */
bool
jdb_cursor_get_next(h, k, szk, d, szd)
     JDB_T              *h;
     char               *k;
     size_t              szk;
     void               *d;
 ...
 **************************************************************************** */
bool
jdb_cursor_set_range(h, k, d, sz, where)
     JDB_T              *h;
     char               *k;
     void               *d;
     size_t              sz;
...
#if USE_BerkeleyDB
#if 0
  if ((h->dbc == NULL) && !jdb_cursor_open(h))
    return FALSE;
#endif
  /*
   **
