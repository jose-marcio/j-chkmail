...
#define CONF_INITIALIZER   {					\
    FALSE, 2., 0, 10000,					\
      "nowhere.j-chkmail.org", "j-chkmail.org",			\
      "1.1.1.1", "nowhere.j-chkmail.org",			\
      "nobody@j-chkmail.org", "grey@j-chkmail.org"		\
}
...
do_test(conf_T * cargs)
{
  ...
  for (n = 0; n < cargs->count; n++)
  {
    ...
    snprintf(buf, sizeof (buf), "grey-%d@j-chkmail.org", rind);
    to = buf;
