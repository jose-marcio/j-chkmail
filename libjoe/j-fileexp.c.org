 ...
 *                                                                            *
 **************************************************************************** */
static bool         check_filename_extension(char *);
     ...
     char               *fname;
{
  ...
  return check_filename_extension(fname);
}
#endif
    ...
    if (ts != NULL)
    {
      ...
      for (i = 0; i < argc; i++)
      {
        ...
        if (strcasecmp(l, "size") == 0)
        {
          ...
          largc = str2tokens(r, 4, largv, ",");

          if (largv[0] != NULL)
            r.min = str2long(largv[0]);
          if (largv[1] != NULL)
            r.max = str2long(largv[1]);

          continue;
        }

        if (strcasecmp(l, "action") == 0)
        {
          if (r != NULL)
          {
            strlcpy(r.action, r, sizeof (r.action));

            continue;
          }

          if (strexpr(l, ".*/.*|ALL", NULL, NULL, TRUE))
          {
            if (*s == '!')
            {
              r.mimecond = FALSE;
    ...
    if (ts != NULL)
    {
           ...
           s = strtok_r(NULL, ";", &ptr))
      {
        ...
        if (strexpr(s, "^ACTION=(notify|discard|reject|ok)$", NULL, NULL, TRUE))
        {
          ...
          if (ps != NULL)
          {
            strlcpy(r.action, ps, sizeof (r.action));
          }
...
bool
check_xfiles(fname, mime, msgsize, action, bufsize)
     char               *fname;
     char               *mime;
     size_t              msgsize;
     char               *action;
...

  {
    ...
    if ((q = (XFILE_T *) j_table_get_first_ptr(&htbl)) != NULL)
    {
      do
      {
        ...
        if (res)
        {
          if (action != NULL && bufsize > 0)
            strlcpy(action, q->action, bufsize);
          break;
        }
